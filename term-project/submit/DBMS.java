...
// TODO : check canceled subscription when user/provider login
	...
	void login() {
		...
		try {
			...
			if(rs.next()) {
				...
				query = "select end_date from user where id=? and datediff(current_date(), end_date)>0";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1,id);
				rs = pstmt.executeQuery();
				if(rs.next()) {
					System.out.println("Hello " + id + ", your subscription has been out of date");
					System.out.println("If you want to resubscribe, type 1");
					String ans = sc.nextLine();
					if(ans.equals("1")) {
						// update start date and end date
						pstmt = conn.prepareStatement("update user set start_date = current_date(), end_date = date_add(current_date(), interval 1 month) where id = ?");
						pstmt.setString(1, id);
						pstmt.executeUpdate();
						System.out.println("You've been resubscribed, your subscription is due to 1 month from today!" + "\nThank you!");
						login = true;
						System.out.println("Hello, " + name + "!");
					}
					else {
						System.out.println("Wrong answer back to menu");
					}
				}
				else {
					login = true;
		...
		if(answer.equals("Y")) {
			try {
				...
				if(rs.next()) {
					...
					//pstmt = conn.prepareStatement("insert into canceled_account values(?,?)");
					//pstmt.setString(1, id);
	...
	void login() {
		...
		try {
			...
			if(rs.next()) {
				...
				query = "select end_date from provider where id=? and datediff(current_date(), end_date)>0";
				pstmt = conn.prepareStatement(query);
				pstmt.setString(1,id);
				rs = pstmt.executeQuery();
				if(rs.next()) {
					...
					if(ans.equals("1")) {
						// update start date and end date
						pstmt = conn.prepareStatement("update provider set start_date = current_date(), end_date = date_add(current_date(), interval 1 month) where id = ?");
						pstmt.setString(1, id);
						pstmt.executeUpdate();
						System.out.println("You've been resubscribed, your subscription is due to 1 month from today!" + "\nThank you!");
						login = true;
	...
	void update() {
		...
		try {
			...
			if(rs.next()) {
				...
				if(ros.length() == 0) ros = "None";
				if(rarch.length() == 0) rarch = "None";
